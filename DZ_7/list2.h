...
class list2
{
	...
	public:
		...
		io_status read (int K1, int K2, avl_tree * garden_name,  avl_tree * garden_phone, FILE *fp = stdin)
		{
			list2_node buf;
			io_status ret;
			...
			*head = (list2_node&&) buf;
			curr = head;
			int index_name = 0, index_phone = 0;
			hash_f(K1, K2, *curr, index_name, index_phone);
			garden_name[index_name].read(curr);
			garden_phone[index_phone].read(curr);
			//garden[res].print();
			//printf("\n");
		...
		list2_node * apply(int K1, int K2, command* test, avl_tree * garden_name, avl_tree * garden_phone, razbor *HELP = nullptr)
		{
			list2_node *curr = head;
			list2_node *head_el = nullptr;
			list2_node *next_el = nullptr;
			//printf("buuu0\n");
			if ((test->get_c_name() == condition::eq || test->get_c_phone() == condition::eq)&& test->get_op() != operation::lor)//when we know the name of evrybody
			{
				//printf("THIS CASE LAND PH N\n");
				int index_n = 0, index_ph = 0;
				hash_f(K1, K2, *test, index_n, index_ph);
				list2_node *end = nullptr;
				//printf("pupupu\n");
				if (test->get_c_name() == condition::eq)
				{
					//printf("hello\n");
		...
		void do_st(int K1, int K2, list2_node *head_el, command* test, avl_tree * garden_name, avl_tree * garden_phone, int& kol)
		{
			...
			if (test->get_type() == command_type::del)
			{
				...
				while(head_el != nullptr)
				{
					...
					if (head == head_el)
					{
						...
						// head_el->print();
						hash_f(K1, K2, *head_el, index_n, index_ph);
						garden_name[index_n].delete_node(head_el);
						garden_phone[index_ph].delete_node(head_el);
						//garden[index].print();
						// printf("NOW:\n");
						// this->print();
						delete head_el;
		...
		void insert(int K1, int K2, command * test, avl_tree * garden_name, avl_tree * garden_phone)
		{
			int index_n = 0, index_ph = 0;
			hash_f(K1, K2, *test, index_n, index_ph);
			list2_node *el = garden_name[index_n].read_record(*test);//find in tree and retern lis2_node where it would lie
